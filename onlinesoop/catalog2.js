  ...
  const renderStars = (rating, ratingCount) => {
    ...
    for (let i = 1; i <= 5; i++) {
      stars.push(
        <Image
          key={i}
          source={i <= rating ? require('./assets/star.png') : require('./assets/star.png')}
  ...
  const renderItem = ({ item, index }) => (
    <View style={styles.productItem}>
      ...
      <TouchableOpacity style={styles.loveButton} onPress={() => toggleLove(index)}>
        <Image
          source={item.loved ? require('./assets/lurus.jpeg') : require('./assets/gr.png')}
    ...
    <View style={styles.container}>
      <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 10 }}>
        <TouchableOpacity onPress={() => navigation.goBack()} style={{ paddingRight: 8 }}>
          <Image source={require('./assets/pink.jpeg')} style={{ width: 20, height: 20, tintColor: '#333333' }} />
        </TouchableOpacity>
